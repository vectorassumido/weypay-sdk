...
| 25 | `/api/webhooks/eupago/` (Webhooks 2.0) está implementado e testado contra o protocolo documentado (35 testes, corpo/HMAC do exemplo oficial), mas **nunca validado contra um payload real** — o corpo exato e o vocabulário de `status` podem ter surpresas, como aconteceu com o `EstadoPedidosJSON` do MB WAY (documentação errada em 3 pontos, só corrigidos por uma chamada real). | `eupago-webhooks.md`, `docs/migration/04-bookwey-security.md` | Configurar o canal Webhooks 2.0 no backoffice EuPago (URL + gerar chave criptográfica), testar com túnel + pagamento real, mesmo padrão já usado para o PINPAY | Não bloqueia produção (falha #1 já fechada de outra forma), mas fica ⚠️ até confirmado |
