...
| 26 | Onde/como obter o `boKey` ("Backoffice key that identifies the merchant account", "key provided by ifthenpay when signing the contract") necessário para `get_order_status()` (item 23) e para a API de ativação de callback por API (`POST /callback/activation`, não usada — o callback do PINPAY já foi ativado manualmente no backoffice)? É uma credencial distinta da `gateway_key` (cria o pagamento) e da chave anti-phishing (valida o callback) — possivelmente o campo "Chave de Backoffice" visto mascarado (`XXXX-XXXX-XXXX-XXXX`) no modal "Ativação de Callback" do backoffice ifthenpay, nunca preenchido nem lido esta sessão. | `ifthenpay-pinpay.md` (j) | O utilizador confirma/obtém o valor junto da ifthenpay ou do próprio backoffice, e configura-o em `Merchant.ifthenpay_bo_key` (campo aditivo já criado, em branco) | Usar `get_order_status()` como reconciliação de fallback do PINPAY em produção — o código já está pronto e ligado condicionalmente (só ativa se o campo estiver preenchido) |
