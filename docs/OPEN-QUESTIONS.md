...
| ~~22~~ | Existe um código dedicado a "pagamento expirado" (janela de tempo passada sem resposta)? | **Não — confirmado deixando um pagamento real expirar deliberadamente.** A ifthenpay devolve `"123"` ("Financial transaction not found"), a mesma resposta de uma transação inexistente. `get_order_status()` mapeia `"123"` → `PaymentStatus.EXPIRED`, assumindo o contrato de uso correto (`IdPedido` sempre devolvido por `request_payment()`, nunca inventado) — ambiguidade teórica com "IdPedido inválido" documentada, não eliminada. Janela real observada: **~4 minutos** (a documentação diz "5 minutos"). Ver `docs/providers/ifthenpay-mbway.md`. |
